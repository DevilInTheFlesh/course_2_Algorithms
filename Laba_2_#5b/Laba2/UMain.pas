...
uses
  ...
  StdCtrls, Buttons, ImgList, System.ImageList, System.Actions;
...
begin
  ...
  if Hash.Count>0 then Grid.RowCount:=Grid.RowCount-1
...
begin
  with FrmEdt do
    begin
      InitInfo(Info);
      if (ShowModal=mrOK)
        then if not(Hash.Add(info))
             then MessageDlg('Добавление невозможноо!' , mtInformation , [mbOk] , 0);
    end;
...
begin
  with FrmEdt do
    begin
      ...
      r:=info;
      if (ShowModal=mrOk)
        then if not(Hash.Delete(r.Key) and Hash.Add(Info))
               then MessageDlg('Невозможно изменить запись!' , mtInformation , [mbOk] , 0);
    end;
end;
  ...
  with FrmFind do
    if ShowModal=mrOk then
      begin
        ...
        else MessageDlg('Ничего не найдено' , mtInformation , [mbOk] , 0 , mbOk);
      end;
end;
